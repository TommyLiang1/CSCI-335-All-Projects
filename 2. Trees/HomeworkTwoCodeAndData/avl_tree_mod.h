// Author: Tommy Liang
// Date: 10/11/2021
// Modified file of the AVL Tree
// Contains the methods to run all of part 2c
...
// void printEnzAcro ( x )--> Finds x and prints its enzyme acronym
// int numNodes()         --> Return the number of nodes
// int avgDepth()         --> Return the average depth of the tree
...
public:
    ...
    /**
     * Prints a certain element's enzyme acronym
     */
    void printEnzAcro(const string &x)
    {
        printEnzAcro(x, root);
    }
...
private:
    ...
    void remove(const Comparable &x, AvlNode *&t, int &counter, int &min_counter)
    {
        ...
        {
            t->element = findMin(t->right, ++min_counter)->element;
     ...
     */
    AvlNode *findMin(AvlNode *t, int &counter) const
    {
        if (t == nullptr)
            return nullptr;
        if (t->left == nullptr)
            return t;
        return findMin(t->left, ++counter);
    }
...
*****************************************************/
    ...
    /*
     * Internal method to print a certain element
     */
    void printEnzAcro(const string &x, AvlNode *t) const
    {
        if (t == nullptr)
            cout << "Not Found\n";
        else if (x < t->element.getRecognitionSeq())
            printEnzAcro(x, t->left);
        else if (t->element.getRecognitionSeq() < x)
            printEnzAcro(x, t->right);
        else
            t->element.printEnzymeAcronyms();
    }
     ...
     * Update heights, then set new root.
     */
    void doubleWithLeftChild(AvlNode *&k3) //Part 2c
    {
        AvlNode *x = k3->left;
        AvlNode *y = x->right;
        x->right = y->left;
        ...
        y->height = max(x->height, k3->height) + 1;
     ...
     * Update heights, then set new root.
     */
    void doubleWithRightChild(AvlNode *&k1) //Part 2c
    {
        AvlNode *y = k1->right;
        AvlNode *x = y->left;
        y->left = x->right;
        ...
        x->height = max(k1->height, y->height) + 1;
